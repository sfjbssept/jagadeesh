...
public class AppControllerTest {
	...
	private ResultActions processApiRequest(String api, HttpMethod methodtype, String name, Employee employee,
		...
		try {
			...
			case GET:
				...
				break;
			case POST:
				response = mockMvc.perform(post(api).header(HttpHeaders.AUTHORIZATION, secret).contentType(MediaType.APPLICATION_JSON).content(asJsonString(employee)).accept(MediaType.APPLICATION_JSON));
	...
	private String asJsonString(Employee employee) {
		...
		try {
			...
			final String jsonContent = mapper.writeValueAsString(employee);
			return jsonContent;
		}catch(Exception e) {
